...
export default function TransferModal({ open, setOpen, onClose, rowData }) {
  const { LotId, WaferNo } = rowData;
  ...
  useEffect(() => {
    ...
    const currentDateTime = getCurrentDateTime();
    const initialFileName = `${LotId}_${WaferNo}_${currentDateTime}`;
    const sanitizedFileName = initialFileName.replace(/:/g, "");
    setFileName(sanitizedFileName);
  }, [LotId, WaferNo]); // LotId 또는 WaferNo 변경될 때만 업데이트
    ...
    <>
      ...
      <div>
        <Dialog open={open} onClose={onClose}>
          ...
          <Dialog open={miniOpen} onClose={handleClose}>
